...
def compare_results(file1: str, file2: str):
    ...
    # 保存类型1：两者都错
    output_file = f"{output_dir}/comparison_v2_both_wrong.jsonl"
    ...
    # 保存类型2：前者对，后者错
    output_file = f"{output_dir}/comparison_v2_file1_right_file2_wrong.jsonl"
    with open(output_file, 'w', encoding='utf-8') as f:
    ...
    # 保存类型3：后者对，前者错
    output_file = f"{output_dir}/comparison_v2_file1_wrong_file2_right.jsonl"
    with open(output_file, 'w', encoding='utf-8') as f:
    ...
    # 保存类型4：两者都对（可选）
    output_file = f"{output_dir}/comparison_v2_both_right.jsonl"
    with open(output_file, 'w', encoding='utf-8') as f:
    ...
    for i, item in enumerate(file1_right_file2_wrong, 1):
        print(f"{i:2d}. {item['name']}: {item['question'][:70]}...")
    
    print("\n【后者对，前者错的样本详细列表】")
    print("=" * 80)
    for i, item in enumerate(file1_wrong_file2_right, 1):
        print(f"{i:2d}. {item['name']}: {item['question'][:70]}...")
...
if __name__ == "__main__":
    file1 = "/mnt/afs/hanzhiyuan/code/HumanOmniV2/affect_r1/output2/affect_r1_grpo2_stage2_2/pope_results_v2.jsonl"
    file2 = "/mnt/afs/hanzhiyuan/code/HumanOmniV2/affect_r1/output2/affect_r1_grpo2_papo23_v2/pope_results_v2.jsonl"
